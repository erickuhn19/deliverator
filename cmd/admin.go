	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		emit("schema", map[string]any{
			...
			"envelope": map[string]any{
				...
				"warnings":      "string[] (rounding, builder fee, partial reads, etc.)",
				"meta":          "object {network,account,weight_used}",
				"degraded_dexs": "string[]? (partial read: sub-dexs whose state/orders were unreadable this call — that data is MISSING, not gone; absent when clean)",
				"truncated":     "bool? (a paged history read stopped at its safety cap with more rows available; absent when complete)",
			},
...
func riskCapValue(c *config.Config, key string) (string, bool) {
	...
	case "risk.drawdown_window_days":
		return strconv.Itoa(r.DrawdownWindowDays), true
